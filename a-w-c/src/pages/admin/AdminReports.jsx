...
function AdminReports() {
  ...
  async function fetchReportData() {
    try {
      ...
      const allAnalyses = analysesSnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
        source: "analysis",
      }));
      ...
      const allData = [...allAnalyses, ...allSubmissions];
  
      // ✅ Filter combined data by date and status
      const filtered = allData.filter((item) => {
        ...
        if (reportType === "completed") {
          return inRange && item.status === "completed";
        } else if (reportType === "pending") {
          return inRange && item.status === "pending";
        }
  
        return inRange; // for "all"
      });
  
      // ✅ Update stats (only for analyses, submissions ignored here if needed)
      ...
      // Separate back into analyses and submissions for reports
      const filteredAnalyses = filtered.filter((item) => item.source === "analysis");
      setReports(filteredAnalyses);
      ...
      // ✅ Chart Data (using combined data)
      setChartData(generateChartData(filtered, startDate, endDate));
      ...
      // ✅ Recent Analyses Table (still showing only from "analyses")
      const recent = [...filteredAnalyses].sort(
        (a, b) =>
          new Date(b.createdAt?.toDate?.() || b.createdAt) -
          new Date(a.createdAt?.toDate?.() || a.createdAt)
      );
      setRecentAnalyses(recent.slice(0, 10));
    } catch (error) {
      console.error("Error fetching report data:", error);
      setError("Failed to load analysis and submission data.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function fetchUserData() {
    try {
      ...
      const activeUsers = usersData.filter((u) => {
        ...
        return lastActivity?.toDate?.() >= thirtyDaysAgo;
      }).length;
      ...
      const newUsers = usersData.filter((u) => {
        return (
          u.createdAt &&
          u.createdAt?.toDate?.() >= startDate &&
          u.createdAt?.toDate?.() <= endDate
        );
      }).length;
  ...
  function exportReports() {
    try {
      ...
      if (activeTab === "analyses") {
        const headers = ["ID", "Title", "User", "Score", "Status", "Created At"];
        csvContent = [
          headers.join(","),
          ...reports.map((report) =>
            [
        ...
        const headers = ["ID", "Name", "Email", "Role", "Created At", "Last Activity"];
        csvContent = [
          headers.join(","),
          ...users.map((user) =>
            [
              ...
              user.role || "user",
              formatDate(user.createdAt).replace(/,/g, " "),
              formatDate(user.lastLogin || user.updatedAt || user.createdAt).replace(/,/g, " "),
            ].join(",")
          ),
        ].join("\n");
  
      } else if (activeTab === "userAnalyses") {
        const headers = [
          "User ID",
          "Name",
          "Email",
        ...
        const headers = ["ID", "User", "Status", "Score", "Created At"];
        csvContent = [
          headers.join(","),
          ...submissions.map((submission) =>
            [
              submission.id,
              (submission.userName || "Unknown").replace(/,/g, " "),
              submission.status || "Unknown",
              submission.score ?? "N/A",
              formatDate(submission.createdAt).replace(/,/g, " "),
            ].join(",")
          ),
        ].join("\n");
      }
      ...
      const fileName =
        activeTab === "analyses"
          ...
          ? `user-analysis-reports-${new Date().toISOString().split("T")[0]}.csv`
          : `submission-reports-${new Date().toISOString().split("T")[0]}.csv`;
