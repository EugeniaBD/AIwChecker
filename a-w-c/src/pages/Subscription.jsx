...
function Subscription() {
  ...
  const handleSelectPlan = async (planName) => {
    if (!currentUser || updating || selectedPlan === planName) return;

    setUpdating(true);
    try {
      const matched = plans.find((p) => p.name === planName);
      const tokens = matched.maxTokens === Infinity
        ? "Unlimited"
        : Math.max(matched.maxTokens - analysisCount, 0);
      ...
      setConfirmation("⚠️ Failed to update plan. Please try again.");
    } finally {
      setUpdating(false);
    }
  };
          ...
          {plans.map((plan, index) => (
            ...
            >
              ...
              <button
                ...
                disabled={updating || selectedPlan === plan.name}
